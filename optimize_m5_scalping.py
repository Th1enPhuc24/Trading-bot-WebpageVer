...
Optimizes SL/TP for M5 timeframe
"""
...
def run_single_backtest(config, network, test_prices, symbol):
    ...
    stats = backtest.run_backtest(
        network=network, prices=test_prices, symbol=symbol,
        initial_balance=10000.0, verbose=False
    )
    return stats


def main():
    TARGET_WIN_RATE = 53.0
    
    print("\n" + "="*70)
    print("🎯 M5 SCALPING OPTIMIZATION")
    print("="*70)
    ...
    # M5 scalping SL/TP grid - smaller values
    sl_tp_configs = [
        (5, 5), (5, 7), (5, 10),
        (7, 7), (7, 10), (7, 14),
        (10, 10), (10, 12), (10, 15),
    ...
    print(f"\n{'SL':<6} {'TP':<6} {'Thresh':<8} {'WinRate':<10} {'Trades':<8} {'PnL':<12}")
    print("-" * 60)
    ...
    for sl, tp in sl_tp_configs:
        for threshold in thresholds:
            ...
            try:
                ...
                status = "✓" if win_rate >= TARGET_WIN_RATE and total_trades >= 20 else ""
    ...
    if valid:
        ...
        print(f"\n🏆 BEST: SL={optimal['sl']}, TP={optimal['tp']}, Thresh={optimal['threshold']}")
